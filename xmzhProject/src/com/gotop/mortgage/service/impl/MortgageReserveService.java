		...
		try {
			...
			if("1".equals(mortgageReserveOut.getOutInType())){
				...
				if("2".equals(mortgageReserveOut.getOperatingMatters()))
				{
					...
					//入库注销产权证  改成外借数量   他项为0
					otherWarrantsNums="0";
					houseWarrantsNums=borrowerNums;
				}
			}
			//外借和领取时,他项数、产权证数都是为0
			if("1".equals(mortgageReserveOut.getOperatingMatters())||"4".equals(mortgageReserveOut.getOperatingMatters())){
				otherWarrantsNums="0";
				//houseWarrantsNums="0";
				houseWarrantsNums=borrowerNums;//产权证本数改为外借数量，手工填写
			}
			...
			if("3".equals(mortgageReserveOut.getOperatingMatters())){
				String warrantsId= mortgageReserveOut.getWarrantsId();
				String mortgageType=mortgageReserveOut.getMortgageType();
